...
### ✅ exp60 — TL-as-tool: teach a small instruct LM to invoke TL closure (DONE — see EXPERIMENTS.md row 60d)

Result: 0.5B LoRA-SFT'd Qwen2.5-Instruct hits **100% (C:sft+tool)** on easy eval and **97% on hard eval** (25–35 person graphs, hops 6–10, distractor relations) with **100%/99% tool-call validity**. Both falsification gates passed (C/A deep-hop ratio ~1.92×; tool-call validity ≥ 95%). (B:sft-no-tool) ≈ (A:base-no-tool) at ~45–48% — the SFT taught delegation, not reasoning; the substrate does all the closure work.

### 💡 exp76 — Multi-relation rule-chain SFT: can the LM compose tools, not just emit one?
- **Hypothesis:** exp65 proved the substrate handles `<tl_rule head="..." body="...">` Datalog-class rules (uncle, cousin, grandparent, great-uncle) via einsum chains over per-relation tensors. exp60d proved a 0.5B LM can learn to emit single `<tl_closure>` tags with 100% validity. Open question: can the same 0.5B model learn to emit *the right `<tl_rule>` for the question being asked* — picking `uncle(X, Y) :- sibling(X, P), parent(P, Y)` for "is alice an uncle of bob?", `cousin(X, Y) :- parent(P, X), sibling(P, Q), parent(Q, Y)` for "are alice and bob cousins?" — across 4–6 distinct rule shapes? End-to-end accuracy via the harness should hit ≥90%, with rule-selection accuracy ≥95%.
- **Falsified if:** Rule-selection accuracy <80% after SFT on ~2k traces (LM confuses uncle vs grand-uncle vs cousin), OR end-to-end accuracy doesn't strictly exceed (C:sft+tool from exp60d) on the same eval queries (i.e., the multi-rule task is no harder than closure, in which case the experiment doesn't discriminate).
- **Smallest test:** Extend exp60a to generate (graph-with-parent+sibling, query, gold-`<tl_rule>`-tag, gold-answer) traces over 5 rule types: grandparent, uncle, aunt, cousin, great-uncle. ~2k train + 400 eval. Reuse exp60d_sft.py with `--eval-file` pointed at the new eval set; harness already supports `<tl_rule>` (exp65). Eval the same 3 conditions: (A) base, (B) SFT-no-tool, (C) SFT + rule harness.
- **Unlocks:** Closes the OPENHUMAN_TL_MEMO §1 multi-hop relational claim end-to-end — not just "the substrate can do uncle" (exp65) and not just "the LM can emit closure tags" (exp60d), but "the LM picks the right rule from a menu and the substrate executes it." This is the real test of LM-as-rule-router.
- **Cost:** 🟡 ~30 min on T4 (2k-trace LoRA SFT + 3-way eval batched). Depends on whether T4 session is still warm — if not, 🟢 on Mac MPS (~45 min).

### 💡 exp60 — TL-as-tool: teach a small instruct LM to invoke TL closure (original entry, preserved for archeology)
